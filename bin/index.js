...
if (cli.flags.file) {
    try {
        cli.input[0] = parseVersionFromString(
            readFileSync(cli.flags.file, 'utf8')
        );
    } catch ({ message }) {
...
if (cli.flags.offline) {
    getUnityUrls(cli.input[0], EDITOR_INSTALLERS_FILE_PATH).then(urls =>
    ...
    updateEditorInstallers(
        ...
        .then(() => getUnityUrls(cli.input[0], EDITOR_INSTALLERS_FILE_PATH))
